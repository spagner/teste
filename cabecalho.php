...
@session_start();
if(@$_SESSION['url_departamento'] == "" ||  @$_GET['nome'] != ""){
	@$_SESSION['url_departamento'] = @$_GET['nome'];
}
$url = @$_SESSION['url_departamento'];
...
if(@$_GET['evento'] != ""){
	...
	$id_ig_ev = $res[0]['departamento'];//erro

	$query = $pdo->query("SELECT * FROM departamentos where id = '$id_ig_ev'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$url = $res[0]['url'];//erro
	@$_SESSION['url_departamento'] = $url;
}

$query = $pdo->query("SELECT * FROM departamentos where url = '$url'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
if(@count($res) > 0){
...
//TOTAIS
$query = $pdo->query("SELECT * FROM beneficiados where departamento = '$id_departamento'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_membros = @count($res);

$query = $pdo->query("SELECT * FROM departamentos");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_departamentos = @count($res);
...
<html>
	...
	<title><?php echo $nome_departamento ?></title>
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<meta name="description" content="Discípulos em Roraima" />
	<meta name="keywords" content="" />
	<meta name="author" content="Gene Charles" />
			...
			<div class="top-menu">
				<div class="row">
					<div class="col-md-12 col-offset-0 text-center">
						<div id="fh5co-logo"><a class="" href="departamento<?php echo $url ?>"><img style="margin-top:-10px; margin-right:8px" src="sistema/img/departamentos/<?php echo $foto_departamento ?>" width="30px"><span class="ml-2"><?php echo mb_strtoupper($nome_departamento) ?></span></a>
					...
					<div class="col-md-12 col-md-offset-0 text-center menu-1">
						<ul>
							<li class="active"><a href="departamento<?php echo $url ?>">Home</a></li>
							...
							<li class="has-dropdown">
								...
								<ul class="dropdown">
									...
									$url = $res[$i]['url'];
									 ?>
									<li><a href="departamento<?php echo $url ?>"><?php echo $nome ?></a></li>
