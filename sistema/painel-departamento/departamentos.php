...
require_once("deslogar-operador.php");
...
<div class="tabela bg-light">
	...
	$query = $pdo->query("SELECT * FROM $pagina where id = '$id_departamento' order by id desc");
		...
		<table id="tabela" class="table table-striped table-light table-hover my-4 my-4" style="width:100%">
			...
			<tbody>
				...
				for($i=0; $i < $total_reg; $i++){
					...
					$query_m = $pdo->query("SELECT * FROM beneficiados where departamento = '$id'");
			...
			$membrosCad = @count($res_m);
					...
					<tr>
						...
						<td class="esc"><?php echo $membrosCad ?> Beneficiados</td>
						<td class="esc"><?php echo $nome_p ?></td>
						<td class="esc"><img src="../img/departamentos/<?php echo $foto ?>" width="30px"></td>
						
						<td>
							<big>
							<a href="#" onclick="editar('<?php echo $id ?>', '<?php echo $nome ?>', '<?php echo $telefone ?>', '<?php echo $endereco ?>', '<?php echo $foto ?>', '<?php echo $diretor ?>', '<?php echo $video ?>', '<?php echo $email ?>', '<?php echo $url ?>', '<?php echo $youtube ?>', '<?php echo $instagram ?>', '<?php echo $facebook ?>', '<?php echo $descricao ?>')" title="Editar Registro">	<i class="bi bi-pencil-square text-primary"></i> </a>
							
							<a href="#" onclick="dados('<?php echo $nome ?>', '<?php echo $telefone ?>', '<?php echo $endereco ?>', '<?php echo $foto ?>', '<?php echo $data_cadF ?>', '<?php echo $matriz ?>', '<?php echo $nome_p ?>', '<?php echo $email ?>', '<?php echo $descricao ?>')" title="Ver Dados">	<i class="bi bi-info-square-fill text-primary"></i> </a>
			...
			<form id="form-ig" method="post">
				...
				<div class="modal-body">
					<div class="row">
						...
						<div class="col-md-4">
							<div class="mb-3">
								<label for="exampleFormControlInput1" class="form-label">Diretor Responsável</label>
								<select class="form-control sel2" id="diretor" name="diretor" style="width:100%;" disabled>
									<?php 
									$query = $pdo->query("SELECT * FROM diretores order by nome asc");
									$res = $query->fetchAll(PDO::FETCH_ASSOC);
									$total_reg = count($res);
									if($total_reg > 0){
						...
						<div class="row">
							<div class="col-md-6">
								...
								<label for="exampleFormControlInput1" class="form-label">Email</label>
									<input type="email" class="form-control" id="email" name="email" placeholder="Email do departamento">			
								</div>					
								</div>
							...
							<div class="mb-3">
								<label for="exampleFormControlInput1" class="form-label">Descrição do departamento <small>(Texto apresentado no site) (Máximo 3000 Caracteres)</small></label>
							...
							<div class="col-md-3">
								<div id="divImg" class="mt-4">
									<img src="../img/departamentos/sem-foto.jpg"  width="130px" id="target">									
								</div>
							</div>
							</div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<span class=""><b>Diretor Responsável:</b> <span id="diretor-dados"></span></span>
					<hr style="margin:4px">
...
<div class="modal fade" id="modalArquivos" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel"><span id="tituloModal">Departamento - <span id="nome-arquivo"></span></span></h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
...
<div class="modal fade" id="modalImagens" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel"><span id="tituloModal">Imagens da Departamento <span id="nome-imagem"></span></span></h5>
				<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			</div>
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-6">
							...
							<div class="col-md-6">
								<div id="divImg" class="mt-4">
									<img src="../img/departamentos/sem-foto.jpg"  width="170px" id="targetlogojpg">									
								</div>
							</div>
						</div>
						...
						<div class="col-md-6">
							...
							<div class="col-md-6">
								<div id="divImg" class="mt-4">
									<img src="../img/departamentos/sem-foto.jpg"  width="170px" id="targetcabjpg">									
								</div>
							</div>
						</div>
					...
					<div class="row">
						<div class="col-md-6">
							...
							<div class="col-md-6">
								<div id="divImg" class="mt-4">
									<img src="../img/departamentos/sem-foto.jpg"  width="170px" id="targetcartjpg">									
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	function editar(id, nome, telefone, endereco, foto, diretor, video, email, url, youtube, instagram, facebook, descricao){
		...
		$('#target').attr('src', '../img/departamentos/' + foto);
		$('#video').val(video);
		$('#email').val(email);
		$('#url').val(url);
		$('#youtube').val(youtube);
	...
	function dados(nome, telefone, endereco, foto, data_cad, matriz, diretor, email, descricao){
		...
		$('#foto-dados').attr('src', '../img/departamentos/' + foto);
		$('#diretor-dados').text(diretor);
	...
	function limpar(){
		...
		document.getElementById("diretor").options.selectedIndex = 0;
		$('#diretor').val($('#diretor').val()).change();


		$('#target').attr('src', '../img/departamentos/sem-foto.jpg');
	}
	...
	function imagens(id, nome, logo, cab, cart){
		...
		$('#targetlogojpg').attr('src', '../img/departamentos/' + logo);
		$('#targetcabjpg').attr('src', '../img/departamentos/' + cab);
		$('#targetcartjpg').attr('src', '../img/departamentos/' + cart);
