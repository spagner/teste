...
<div class="col-md-12 my-3">
	<a href="#" onclick="inserir()" type="button" class="btn btn-dark btn-sm">New Membro</a>
</div>
			...
			<tbody>
				...
				for($i=0; $i < $total_reg; $i++){
					...
					<tr class="<?php echo $inativa ?>">
						...
						<td>
							<big>
								<a href="#" onclick="editar('<?php echo $id ?>', '<?php echo $nome ?>', '<?php echo $cpf ?>', '<?php echo $email ?>', '<?php echo $telefone ?>', '<?php echo $endereco ?>', '<?php echo $foto ?>', '<?php echo $data_nasc ?>', '<?php echo $igreja ?>', '<?php echo $nome_ig ?>', '<?php echo $data_bat ?>', '<?php echo $cargo ?>', '<?php echo $estado ?>')" title="Editar Registro">	<i class="bi bi-pencil-square text-primary"></i> </a>
...
<!--
				...
				<script type="text/javascript">
					...
					function editar(id, nome, cpf, email, telefone, endereco, foto, data_nasc, igreja, nome_ig, data_bat, cargo, estado){
						$('#id').val(id);
						$('#nome').val(nome);
						$('#email').val(email);
						$('#cpf').val(cpf);
