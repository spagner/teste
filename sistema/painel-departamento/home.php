...
$query = $pdo->query("SELECT * FROM beneficiados where departamento = '$id_departamento' and ativo = 'Sim'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$membrosCadastrados = @count($res);

$query = $pdo->query("SELECT * FROM pagar where departamento = '$id_departamento' and vencimento = curDate() and pago != 'Sim'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$pagarHoje = @count($res);

$query = $pdo->query("SELECT * FROM receber where departamento = '$id_departamento' and vencimento = curDate() and pago != 'Sim'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
...
$query = $pdo->query("SELECT * FROM grupos where departamento = '$id_departamento'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$gruposCadastrados = @count($res);

$query = $pdo->query("SELECT * FROM celulas where departamento = '$id_departamento'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$celulasCadastradas = @count($res);

$query = $pdo->query("SELECT * FROM patrimonios where departamento_cad = '$id_departamento' and ativo = 'Sim'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
...
$query = $pdo->query("SELECT * FROM movimentacoes where departamento = '$id_departamento' and data >= '$dataInicioMes' and data <= curDate()");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
if($total_reg > 0){
...
$dataMes = Date('m');
$dataDia = Date('d');
$query = $pdo->query("SELECT * FROM beneficiados where departamento = '$id_departamento' and month(data_nasc) = '$dataMes' and day(data_nasc) = '$dataDia' order by data_nasc asc, id desc");
	$query_diretores = $pdo->query("SELECT * FROM diretores where departamento = '$id_departamento' and month(data_nasc) = '$dataMes' and day(data_nasc) = '$dataDia' order by data_nasc asc, id desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = count($res);
$res_diretores = $query_diretores->fetchAll(PDO::FETCH_ASSOC);
	$total_reg_diretores = count($res_diretores);
$totalAniversariantes = $total_reg + $total_reg_diretores;
	...
	<section id="minimal-statistics" style="margin-right:20px">
		...
		<div class="row mb-2 <?php echo $esc_secretario ?>">
			<div class="col-8 mt-3 mb-1">
				<h4 class="text-uppercase textocinzaescuro"><small><small><?php echo $nome_departamento ?> </small></small></h4>
		...
		<div class="row mb-4">
			...
			<div class="col-xl-3 col-sm-6 col-12 mb-2 linkcard <?php echo $esc_tesoureiro ?>"> 
				<a href="index.php?pag=beneficiados">
					<div class="card">
						<div class="card-content">
							<div class="card-body">
								<div class="row">
									...
									<div class="col-9 text-end">
										...
										<span class="textocinzaescuro">Total de Beneficiados</span>
									</div>
								</div>
							</div>
						</div>
...
<!--
	...
	<?php 
	$query = $pdo->query("SELECT * FROM tarefas where status = 'Agendada' and departamento = '$id_departamento' order by status asc, data asc, hora asc LIMIT $quantidade_tarefas");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = count($res);
	?>
