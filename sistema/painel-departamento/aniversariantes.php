...
if(@$_GET['filtrar'] == "dia"){
	...
	//trocar para todos os funcionarios
	...
	$query_diretores = $pdo->query("SELECT * FROM diretores where id != 1 and departamento = '$id_departamento' and month(data_nasc) = '$dataMes' and day(data_nasc) = '$dataDia' order by data_nasc asc, id desc");
	
}else{
	$classe_mes = 'text-primary';
	$classe_dia = 'text-dark';

	$query = $pdo->query("SELECT * FROM beneficiados where departamento = '$id_departamento' and month(data_nasc) = '$dataMes' order by data_nasc asc, id desc");

	$query_diretores = $pdo->query("SELECT * FROM diretores where id != 1 and departamento = '$id_departamento' and month(data_nasc) = '$dataMes' order by data_nasc asc, id desc");
...
<div class="tabela bg-light">
	...
	if($total_reg > 0 || $total_reg_diretores > 0){
		...
		<table id="example" class="table table-striped table-light table-hover my-4 my-4" style="width:100%">
			...
			<tbody>
				...
				for($i=0; $i < $total_reg_diretores; $i++){
					foreach ($res_diretores[$i] as $key => $value){} 

						$nome = $res_diretores[$i]['nome'];
					$cpf = $res_diretores[$i]['cpf'];
					$email = $res_diretores[$i]['email'];
					$telefone = $res_diretores[$i]['telefone'];
					$endereco = $res_diretores[$i]['endereco'];
					$foto = $res_diretores[$i]['foto'];
					$data_nasc = $res_diretores[$i]['data_nasc'];
					$data_cad = $res_diretores[$i]['data_cad'];
					$obs = $res_diretores[$i]['obs'];
					$igreja = $res_diretores[$i]['igreja'];
					$id = $res_diretores[$i]['id'];
	...
	<?php }else{
		echo 'Não Existem Funcionários Aniversáriando Este Mês';
	} ?>
</div>
