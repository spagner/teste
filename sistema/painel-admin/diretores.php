...
<div class="col-md-12 my-3">
	<a href="#" onclick="inserir()" type="button" class="btn btn-dark btn-sm">Novo Diretor</a>
</div>

<div class="tabela bg-light">
	<?php 

	$query = $pdo->query("SELECT * FROM $pagina where id != 1 order by id desc");
		...
		<table id="example" class="table table-striped table-light table-hover my-4 my-4" style="width:100%">
			...
			<tbody>
				...
				for($i=0; $i < $total_reg; $i++){
					...
					if(count($res_con) > 0){
						$nome_dp = $res_con[0]['nome'];
					}else{
						$nome_dp = $nome_departamento_sistema;
					}
					...
					<tr>
						...
						<td class="esc"><?php echo $nome_dp ?></td>
						
						<td class="esc"><img src="../img/funcionarios/<?php echo $foto ?>" width="30px"></td>
						
						<td>
							<a href="#" onclick="editar('<?php echo $id ?>', '<?php echo $nome ?>', '<?php echo $cpf ?>', '<?php echo $email ?>', '<?php echo $telefone ?>', '<?php echo $endereco ?>', '<?php echo $foto ?>', '<?php echo $data_nasc ?>', '<?php echo $departamento ?>', '<?php echo $nome_dp ?>', '<?php echo $prebenda ?>')" title="Editar Registro">	<i class="bi bi-pencil-square text-primary"></i> </a>

							<a href="#" onclick="excluir('<?php echo $id ?>' , '<?php echo $nome ?>')" title="Excluir Registro">	<i class="bi bi-trash text-danger"></i> </a>
							<a href="#" onclick="dados('<?php echo $nome ?>', '<?php echo $cpf ?>', '<?php echo $email ?>', '<?php echo $telefone ?>', '<?php echo $endereco ?>', '<?php echo $foto ?>', '<?php echo $data_nascF ?>', '<?php echo $data_cadF ?>', '<?php echo $nome_dp ?>', '<?php echo $prebenda ?>')" title="Ver Dados">	<i class="bi bi-info-square-fill text-primary"></i> </a>
					...
					<div class="row">
						<div class="col-md-4">
							<div class="mb-3">
								...
								<select class="form-control sel2" id="departamento" name="departamento" style="width:100%;" required>
									<?php 
							...
							<div class="col-md-3">
								<div id="divImg" class="mt-4">
									<img src="../img/funcionarios/sem-foto.jpg"  width="130px" id="target">									
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	function editar(id, nome, cpf, email, telefone, endereco, foto, data_nasc, departamento, nome_dp, prebenda){
		$('#id').val(id);
		$('#nome').val(nome);
		$('#email').val(email);
		$('#cpf').val(cpf);
		...
		$('#target').attr('src', '../img/funcionarios/' + foto);
	...
	function dados(nome, cpf, email, telefone, endereco, foto, data_nasc, data_cad, departamento, prebenda){
		...
		$('#foto-dados').attr('src', '../img/funcionarios/' + foto);
	...
	function limpar(){
		...
		document.getElementById("departamento").options.selectedIndex = 0;
		$('#departamento').val($('#departamento').val()).change();

		$('#target').attr('src', '../img/funcionarios/sem-foto.jpg');
	}
