...
if(@count($res) > 0 and $id_reg != $id){
	echo 'O Nome do departamento já está cadastrado!';
	exit();
}
...
$id_reg = @$res[0]['id'];
if(@count($res) > 0 and $id_reg != $id){
	echo 'O Email do departamento já está cadastrado!';
	exit();
}
...
$caminho = '../../img/departamentos/' .$nome_img;
...
if($id == "" || $id == 0){
	$query = $pdo->prepare("INSERT INTO $pagina SET nome = :nome, telefone = :telefone, endereco = :endereco, imagem = '$imagem', data_cad = curDate(), matriz = 'Não', diretor = '$diretor', logo_rel = 'sem-foto.jpg', cab_rel = 'sem-foto.jpg', carteirinha_rel = 'sem-foto.jpg', video = :video, email = :email, url = :url, youtube = :youtube, instagram = :instagram, facebook = :facebook, descricao = :descricao, prebenda = '$prebenda'");


}else{
	if($imagem == "sem-foto.jpg"){
		$query = $pdo->prepare("UPDATE $pagina SET nome = :nome, telefone = :telefone, endereco = :endereco, diretor = '$diretor', video = :video, email = :email, url = :url, youtube = :youtube, instagram = :instagram, facebook = :facebook, descricao = :descricao, prebenda = '$prebenda' where id = '$id'");
	}else{
		...
		$query = $pdo->prepare("UPDATE $pagina SET nome = :nome, telefone = :telefone, endereco = :endereco, imagem = '$imagem', diretor = '$diretor', video = :video, email = :email, url = :url, youtube = :youtube, instagram = :instagram, facebook = :facebook, descricao = :descricao, prebenda = '$prebenda' where id = '$id'");
	}
