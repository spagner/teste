...
<div class="tabela bg-light my-3">
		...
		<table id="example" class="table table-striped table-light table-hover my-4 my-4" style="width:100%">
			...
			<tbody>
				...
				for($i=0; $i < $total_reg; $i++){
					...
					$foto = $res[$i]['foto'];
					$departamento_cad = $res[$i]['departamento_cad'];
					$data_cad = $res[$i]['data_cad'];
					$obs = $res[$i]['obs'];
					$departamento_item = $res[$i]['departamento_item'];
					...
					$query_con = $pdo->query("SELECT * FROM departamentos where id = '$departamento_cad'");
					$res_con = $query_con->fetchAll(PDO::FETCH_ASSOC);
					if(count($res_con) > 0){
					...
					$query_con = $pdo->query("SELECT * FROM departamentos where id = '$departamento_item'");
					$res_con = $query_con->fetchAll(PDO::FETCH_ASSOC);
					if(count($res_con) > 0){
				...
				<div class="modal-body">
					...
					<div class="row">
						...
						<div class="col-md-6">
							...
							<span class=""><b>Departamento Dona Item:</b> <span id="departamento-cad-dados"></span></span>
						</div>
					</div>
					<hr style="margin:4px">
					...
					<div class="row">
						<div class="col-md-6">
							<span class=""><b>Departamento Possui Item:</b> <span id="departamento-item-dados"></span></span>
						</div>
				...
				<div class="modal-content">
					...
					<form id="form-devolver" method="post">
						<div class="modal-body">
							...
							<input type="hidden" class="form-control" name="departamento" id="id-ig" >
	...
	<script type="text/javascript">
		...
		function dados(codigo, nome, descricao, valor, foto, usuario_cad, data_cad, departamento_cad, departamento_item, usuario_emp, data_emp, ativo, obs, entrada, doador){
			...
			$('#departamento-cad-dados').text(departamento_cad);
			$('#departamento-item-dados').text(departamento_item);
...
function devolver(id, nome, departamento){
    $('#id-devolver').val(id);
    $('#id-ig').val(departamento);
