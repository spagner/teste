...
<div class="row" style="margin:10px">
				...
				<?php 
						$query = $pdo->query("SELECT * FROM eventos where departamento = '$id_departamento' and ativo = 'Sim' and tipo = 'Evento' order by  data_evento desc, id desc LIMIT $limite, $itens_por_pagina");
						...
						if($total_reg > 0){
							for($i=0; $i < $total_reg; $i++){
								...
								if($data_evento >= date('Y-m-d')){
									...
									//totalizar páginas
									$query_cont = $pdo->query("SELECT * FROM eventos where departamento = '$id_departamento' and ativo = 'Sim' and tipo = 'Evento'");
